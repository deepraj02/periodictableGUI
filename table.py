...
def exe():
    cncl=tmsg.askquestion("Confirm Exit Message","Do You Really want to Exit??")
    if cncl=="yes":
        quit()
...
textentry = Entry(root, width=30,borderwidth=5,font="firacode 18", fg="#000000")
textentry.pack(padx=10,pady=10,ipady=14,ipadx=10,fill=X)

# add a submit button
Button(root, text="SUBMIT",font="Comicsansms 16", width=9, command=click).pack(padx=10,pady=10,anchor=W)

Button(root, text="EXIT",font="Comicsansms 16",width=9, command=exe).pack(padx=10,pady=10,anchor=NE)

# create another label
Label(root, text="\nResults:",font="Hack 18").pack()

# create a text box
output = Text(root, width=75, height=10, wrap=WORD, background="white",font="Firacode 17")
output.pack()
